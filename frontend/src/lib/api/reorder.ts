...
export const reorderApi = {
  ...
  getReorderSuggestions: async (params?: ApiQueryParams) => {
    const response = await apiClient.get<PaginatedResponse<ReorderSuggestion>>("/inventory/reorder-suggestions", {
      params,
    });
    return response.data;
  },
  ...
  getReorderAlerts: async (params?: ApiQueryParams) => {
    const response = await apiClient.get<PaginatedResponse<ReorderAlert>>("/inventory/reorder-alerts", {
      params,
    });
    return response.data;
  },
  ...
  acknowledgeAlerts: async (data: AcknowledgeAlertInput) => {
    const response = await apiClient.post("/inventory/reorder-alerts/acknowledge", data);
    return response.data;
  },

  // Statistics
  getReorderStatistics: async () => {
    const response = await apiClient.get<ReorderStatistics>("/inventory/reorder-statistics");
    return response.data;
  },
