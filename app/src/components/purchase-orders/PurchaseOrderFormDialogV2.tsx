...
interface PurchaseOrderFormDialogV2Props {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  purchaseOrder?: PurchaseOrder | null;
}

export function PurchaseOrderFormDialogV2({
  open,
  onOpenChange,
  purchaseOrder,
}: PurchaseOrderFormDialogV2Props) {
  ...
  const totals = useMemo(() => {
    ...
    const totalTax = lineItems.reduce((sum, item) => {
      ...
      return sum + (taxableAmount * (item.taxPercent || 0)) / 100;
    }, 0);
