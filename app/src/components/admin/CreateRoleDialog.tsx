  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (copyFromRoleId && copyFromRoleData?.permissions && copyFromRoleData.permissions.length > 0) {
        ...
        toast.success(`Role "${name}" created with permissions copied from "${copyFromRoleData.name}"`);
      } else {
          ...
          <div className="space-y-2">
            ...
            {copyFromRoleId && copyFromRoleData && (
              <p className="text-xs text-muted-foreground">
                Will copy {copyFromRoleData.permissions?.length || 0} permission(s) from &quot;{copyFromRoleData.name}&quot;
              </p>
            )}
          </div>
