...
export async function generateReceiptPDF(transaction: POSTransaction): Promise<string> {
  ...
  const addTwoColumn = (label: string, value: string, y: number, fontSize: number = 9, isBold: boolean = false) => {
    doc.setFontSize(fontSize);
    doc.setFont('helvetica', isBold ? 'bold' : 'normal');
  ...
  addLeftText(`Date: ${formatDate(transaction.transactionDate)}  ${formatTime(transaction.transactionDate)}`, yPos, 9);
  yPos += smallLineHeight;
  ...
  // ===== PAYMENT =====
  const paymentMethod = transaction.payments.length > 0
