        ...
        <div className="space-y-3">
          {isLoading ? (
            ...
            filteredMovements.map((movement) => (
              <Card key={movement.itemId} className="hover:shadow-md transition-shadow">
                <CardContent className="p-4">
                  {/* Item Header */}
                  <div className="font-semibold text-base mb-1">{movement.itemName}</div>
                  <div className="text-xs text-gray-500 mb-3">{movement.itemCode}</div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between py-2 border-b">
                      ...
                      <span className="font-semibold">{movement.beginningQty.toFixed(2)} {movement.uomName}</span>
                    </div>
                    ...
                    {movement.itemsIn > 0 && (
                      <div className="flex items-center justify-between py-2">
                        ...
                        <span className="font-semibold text-green-600">+{movement.itemsIn.toFixed(2)} {movement.uomName}</span>
                      </div>
                    )}
                    ...
                    {movement.itemsOut > 0 && (
                      <div className="flex items-center justify-between py-2">
                        ...
                        <span className="font-semibold text-red-600">-{movement.itemsOut.toFixed(2)} {movement.uomName}</span>
                      </div>
                    )}
                    ...
                    {movement.itemsIn === 0 && movement.itemsOut === 0 && (
                      <div className="py-2 text-center">
                        <span className="text-sm text-gray-400 italic">No movement on this date</span>
                      </div>
                    )}

                    {/* Ending */}
                    <div className="flex items-center justify-between py-2 border-t bg-blue-50 -mx-4 px-4 rounded">
                      <span className="text-sm font-semibold text-blue-900">Ending Inventory</span>
                      <span className="font-bold text-blue-900">{movement.endingQty.toFixed(2)} {movement.uomName}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
