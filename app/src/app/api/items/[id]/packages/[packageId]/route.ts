...
export async function PUT(
  ...
  try {
    ...
    if (isBasePackage && body.qtyPerPack && parseFloat(body.qtyPerPack) !== 1.0) {
      return NextResponse.json(
        { error: 'Cannot change qtyPerPack of base package. Base package must have qtyPerPack = 1.0' },
        { status: 400 }
      );
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (packageId === item?.package_id) {
      return NextResponse.json(
        { error: 'Cannot delete the base package. This package is set as the item\'s base storage unit.' },
        { status: 400 }
      );
    }
