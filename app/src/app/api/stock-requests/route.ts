...
export async function GET(request: NextRequest) {
  try {
    ...
    // Apply filters
    if (search) {
      query = query.or(`request_code.ilike.%${search}%,purpose.ilike.%${search}%,department.ilike.%${search}%`)
    }
    if (fromLocationId) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const requestedByName = [userData.first_name, userData.last_name]
      .filter(Boolean)
      .join(' ') || userData.email || user.email
    ...
    const requestItems = body.items.map((item: any) => ({
