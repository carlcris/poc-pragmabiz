...
// POST /api/stock-requests/[id]/ready-for-pick - Mark stock request as ready for picking
  ...
  try {
    ...
    if (existingRequest.status !== 'approved') {
      return NextResponse.json(
        { error: 'Only approved stock requests can be marked ready for pick' },
        { status: 400 }
      )
    }
    ...
    const { data: destinationWarehouse } = await supabase
      .from('stock_requests')
      .select('destination_warehouse_id, destination_warehouse:warehouses!stock_requests_destination_warehouse_id_fkey(business_unit_id)')
      .eq('id', id)
      .single()

    const destinationBusinessUnitId = destinationWarehouse?.destination_warehouse?.business_unit_id || null

    if (!destinationBusinessUnitId || destinationBusinessUnitId !== currentBusinessUnitId) {
      return NextResponse.json(
        { error: 'Only the fulfillment business unit can mark ready for pick' },
        { status: 403 }
      )
    }

    // Update status to ready_for_pick
    ...
    if (updateError) {
      console.error('Error marking stock request ready for pick:', updateError)
    ...
    console.error('Error in stock-request ready-for-pick:', error)
