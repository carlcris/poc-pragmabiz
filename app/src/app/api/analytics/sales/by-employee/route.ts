...
export const GET = async (req: NextRequest) => {
  try {
    ...
    const employeeStats = invoices?.reduce((acc: Record<string, { id: string; totalSales: number; totalCommission: number; count: number }>, inv: { primary_employee_id: string; total_amount: number; commission_total: number }) => {
    ...
    const territoriesByEmployee = territories?.reduce((acc: Record<string, string[]>, territory: { employee_id: string; city: string; region_state: string }) => {
      const empId = territory.employee_id;
      if (!acc[empId]) {
        acc[empId] = [];
      }
      const territoryStr = territory.city ? `${territory.city}, ${territory.region_state}` : territory.region_state;
