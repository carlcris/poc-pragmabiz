...
export default function StockRequestsPage() {
  ...
  const hasRowActions = (request: StockRequest) => {
    ...
    if ((request.status === "delivered" || request.status === "picked") && canReceiveRequest(request)) {
      return true;
    }
  ...
  const getActionDialogContent = () => {
    const actionLabels: Record<string, { title: string; description: string; confirmText: string; confirmClass: string; needsReason: boolean }> = {
      submit: {
      ...
      complete: {
        ...
        description: "Complete this stock request? This will create stock transactions and update inventory levels.",
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Manage stock requests and fulfillment workflow
            </p>
          </div>
                                  ...
                                  <DropdownMenuContent align="end">
                                    {request.status === "draft" && (
                                      <>
                                        ...
                                        <DropdownMenuSeparator />
                                        <DropdownMenuItem onClick={() => handleAction("submit", request)}>
                                    ...
                                    {request.status === "approved" && canFulfillRequest(request) && (
                                      <DropdownMenuItem onClick={() => handleAction("ready_for_pick", request)}>
                                        <Package className="h-4 w-4 mr-2" />
                                        Ready to Pick
                                      </DropdownMenuItem>
                                    )}

                                    {request.status === "ready_for_pick" && canFulfillRequest(request) && (
                                      <DropdownMenuItem onClick={() => handleAction("deliver", request)}>
                                    ...
                                    {(request.status === "delivered" || request.status === "picked") &&
                                      canReceiveRequest(request) && (
                                        <DropdownMenuItem onClick={() => handleAction("receive", request)}>
                                          <CheckCircle className="h-4 w-4 mr-2" />
                                          Receive
                                        </DropdownMenuItem>
                                      )}
                          ...
                          <Table>
                            <TableHeader>
                              <TableRow>
                                ...
                                <TableHead className="text-xs py-2 w-[80px]">Actions</TableHead>
                              </TableRow>
                            </TableHeader>
                            <TableBody>
