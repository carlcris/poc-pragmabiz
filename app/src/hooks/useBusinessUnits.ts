...
export function useBusinessUnits() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiClient.get<BusinessUnitsResponse>(
        '/api/business-units'
      );
...
export function useSetBusinessUnitContext(options?: { silent?: boolean }) {
  ...
  return useMutation({
    mutationFn: async (businessUnitId: string) => {
      ...
      const response = await apiClient.post<SetContextResponse>(
        '/api/business-units/set-context',
        {
...
export function useInitializeBusinessUnit() {
  ...
  const { currentBusinessUnit, getDefaultBusinessUnit } =
