-- Migration: Add nullable item import cost fields for purchasing defaults.
-- Date: 2026-05-06

BEGIN;

ALTER TABLE public.items
  ADD COLUMN IF NOT EXISTS import_cost NUMERIC(20, 4),
  ADD COLUMN IF NOT EXISTS import_currency VARCHAR(3);

ALTER TABLE public.items
  DROP CONSTRAINT IF EXISTS chk_items_import_cost_non_negative,
  ADD CONSTRAINT chk_items_import_cost_non_negative
    CHECK (import_cost IS NULL OR import_cost >= 0);

ALTER TABLE public.items
  DROP CONSTRAINT IF EXISTS chk_items_import_currency_required,
  ADD CONSTRAINT chk_items_import_currency_required
    CHECK (import_cost IS NULL OR import_currency IS NOT NULL);

ALTER TABLE public.items
  DROP CONSTRAINT IF EXISTS chk_items_import_currency_format,
  ADD CONSTRAINT chk_items_import_currency_format
    CHECK (import_currency IS NULL OR import_currency ~ '^[A-Z]{3}$');
