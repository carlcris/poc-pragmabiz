-- ============================================================================
-- Migration: Add item_unit_option_id to delivery_note_items
-- Version: 20260405143000
-- Description: Persists the exact item-specific unit option selected on
--              delivery note lines. Historical rows are not backfilled.
-- Date: 2026-04-05
-- ============================================================================
...
COMMENT ON COLUMN public.delivery_note_items.item_unit_option_id IS
  'Selected item-specific unit option for the delivery note line.';

CREATE INDEX IF NOT EXISTS idx_delivery_note_items_item_unit_option_id
  ON public.delivery_note_items(item_unit_option_id)
  WHERE item_unit_option_id IS NOT NULL;

CREATE OR REPLACE FUNCTION public.validate_delivery_note_item_unit_option()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
DROP TRIGGER IF EXISTS trigger_delivery_note_items_validate_item_unit_option ON public.delivery_note_items;
CREATE TRIGGER trigger_delivery_note_items_validate_item_unit_option
  BEFORE INSERT OR UPDATE ON public.delivery_note_items
  FOR EACH ROW
  EXECUTE FUNCTION public.validate_delivery_note_item_unit_option();
