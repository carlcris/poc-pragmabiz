import { getUomIdByCode } from "./uom";

export interface Item {
  id: string;
  companyId: string;
  code: string;
  name: string;
  description: string;
  itemType: "goods" | "service" | "raw_material" | "finished_goods";
  uom: string;
  uomId: string; // Added for API compatibility
  category: string;
  standardCost: number;
  listPrice: number;
  reorderLevel: number;
  reorderQty: number;
  isActive: boolean;
  createdAt: string;
  updatedAt: string;
}

export const mockItems: Item[] = [
  {
    id: "item-1",
    companyId: "company-1",
    code: "ITEM-001",
    name: "Laptop Dell XPS 15",
    description: "High-performance laptop for business use",
    itemType: "goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Electronics",
    standardCost: 1200.0,
    listPrice: 1800.0,
    reorderLevel: 10,
    reorderQty: 20,
    isActive: true,
    createdAt: "2024-01-15T10:00:00Z",
    updatedAt: "2024-01-15T10:00:00Z",
  },
  {
    id: "item-2",
    companyId: "company-1",
    code: "ITEM-002",
    name: "Office Chair Executive",
    description: "Ergonomic office chair with lumbar support",
    itemType: "goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Furniture",
    standardCost: 150.0,
    listPrice: 300.0,
    reorderLevel: 5,
    reorderQty: 15,
    isActive: true,
    createdAt: "2024-01-16T11:30:00Z",
    updatedAt: "2024-01-16T11:30:00Z",
  },
  {
    id: "item-3",
    companyId: "company-1",
    code: "ITEM-003",
    name: "Software License - Office Suite",
    description: "Annual software license for productivity suite",
    itemType: "service",
    uom: "LICENSE",
    uomId: getUomIdByCode("LICENSE"),
    category: "Software",
    standardCost: 80.0,
    listPrice: 150.0,
    reorderLevel: 0,
    reorderQty: 0,
    isActive: true,
    createdAt: "2024-01-17T09:00:00Z",
    updatedAt: "2024-01-17T09:00:00Z",
  },
  {
    id: "item-4",
    companyId: "company-1",
    code: "ITEM-004",
    name: "Steel Sheet 2mm",
    description: "Raw material for manufacturing",
    itemType: "raw_material",
    uom: "SQM",
    uomId: getUomIdByCode("SQM"),
    category: "Raw Materials",
    standardCost: 25.0,
    listPrice: 40.0,
    reorderLevel: 100,
    reorderQty: 500,
    isActive: true,
    createdAt: "2024-01-18T14:20:00Z",
    updatedAt: "2024-01-18T14:20:00Z",
  },
  {
    id: "item-5",
    companyId: "company-1",
    code: "ITEM-005",
    name: "Custom Cabinet Unit",
    description: "Finished product - assembled cabinet",
    itemType: "finished_goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Furniture",
    standardCost: 200.0,
    listPrice: 450.0,
    reorderLevel: 8,
    reorderQty: 20,
    isActive: true,
    createdAt: "2024-01-19T16:45:00Z",
    updatedAt: "2024-01-19T16:45:00Z",
  },
  {
    id: "item-6",
    companyId: "company-1",
    code: "ITEM-006",
    name: "Wireless Mouse Logitech MX Master",
    description: "Premium wireless mouse with precision tracking",
    itemType: "goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Electronics",
    standardCost: 45.0,
    listPrice: 99.0,
    reorderLevel: 15,
    reorderQty: 30,
    isActive: true,
    createdAt: "2024-01-20T08:00:00Z",
    updatedAt: "2024-01-20T08:00:00Z",
  },
  {
    id: "item-7",
    companyId: "company-1",
    code: "ITEM-007",
    name: "Standing Desk Electric",
    description: "Height adjustable electric standing desk",
    itemType: "goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Furniture",
    standardCost: 350.0,
    listPrice: 699.0,
    reorderLevel: 3,
    reorderQty: 10,
    isActive: true,
    createdAt: "2024-01-21T09:15:00Z",
    updatedAt: "2024-01-21T09:15:00Z",
  },
  {
    id: "item-8",
    companyId: "company-1",
    code: "ITEM-008",
    name: "LED Monitor 27 inch 4K",
    description: "Ultra HD 4K monitor for professional use",
    itemType: "goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Electronics",
    standardCost: 280.0,
    listPrice: 549.0,
    reorderLevel: 8,
    reorderQty: 15,
    isActive: true,
    createdAt: "2024-01-22T10:30:00Z",
    updatedAt: "2024-01-22T10:30:00Z",
  },
  {
    id: "item-9",
    companyId: "company-1",
    code: "ITEM-009",
    name: "Aluminum Sheet 3mm",
    description: "Lightweight aluminum for manufacturing",
    itemType: "raw_material",
    uom: "SQM",
    uomId: getUomIdByCode("SQM"),
    category: "Raw Materials",
    standardCost: 35.0,
    listPrice: 55.0,
    reorderLevel: 80,
    reorderQty: 400,
    isActive: true,
    createdAt: "2024-01-23T11:45:00Z",
    updatedAt: "2024-01-23T11:45:00Z",
  },
  {
    id: "item-10",
    companyId: "company-1",
    code: "ITEM-010",
    name: "Cloud Storage Service",
    description: "Monthly cloud storage subscription",
    itemType: "service",
    uom: "LICENSE",
    uomId: getUomIdByCode("LICENSE"),
    category: "Software",
    standardCost: 15.0,
    listPrice: 29.99,
    reorderLevel: 0,
    reorderQty: 0,
    isActive: true,
    createdAt: "2024-01-24T13:00:00Z",
    updatedAt: "2024-01-24T13:00:00Z",
  },
  {
    id: "item-11",
    companyId: "company-1",
    code: "ITEM-011",
    name: "Mechanical Keyboard RGB",
    description: "Gaming keyboard with RGB lighting",
    itemType: "goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Electronics",
    standardCost: 65.0,
    listPrice: 129.0,
    reorderLevel: 12,
    reorderQty: 25,
    isActive: true,
    createdAt: "2024-01-25T14:15:00Z",
    updatedAt: "2024-01-25T14:15:00Z",
  },
  {
    id: "item-12",
    companyId: "company-1",
    code: "ITEM-012",
    name: "Bookshelf 5-Tier",
    description: "Wooden bookshelf with 5 shelves",
    itemType: "finished_goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Furniture",
    standardCost: 85.0,
    listPrice: 179.0,
    reorderLevel: 6,
    reorderQty: 15,
    isActive: true,
    createdAt: "2024-01-26T15:30:00Z",
    updatedAt: "2024-01-26T15:30:00Z",
  },
  {
    id: "item-13",
    companyId: "company-1",
    code: "ITEM-013",
    name: "USB-C Hub 7-in-1",
    description: "Multi-port USB-C hub adapter",
    itemType: "goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Electronics",
    standardCost: 22.0,
    listPrice: 49.0,
    reorderLevel: 20,
    reorderQty: 40,
    isActive: true,
    createdAt: "2024-01-27T16:45:00Z",
    updatedAt: "2024-01-27T16:45:00Z",
  },
  {
    id: "item-14",
    companyId: "company-1",
    code: "ITEM-014",
    name: "Copper Wire 2.5mm",
    description: "Electrical copper wire",
    itemType: "raw_material",
    uom: "M",
    uomId: getUomIdByCode("M"),
    category: "Raw Materials",
    standardCost: 2.5,
    listPrice: 4.5,
    reorderLevel: 500,
    reorderQty: 2000,
    isActive: true,
    createdAt: "2024-01-28T08:00:00Z",
    updatedAt: "2024-01-28T08:00:00Z",
  },
  {
    id: "item-15",
    companyId: "company-1",
    code: "ITEM-015",
    name: "Webcam 1080p HD",
    description: "Full HD webcam with microphone",
    itemType: "goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Electronics",
    standardCost: 38.0,
    listPrice: 79.0,
    reorderLevel: 10,
    reorderQty: 20,
    isActive: true,
    createdAt: "2024-01-29T09:15:00Z",
    updatedAt: "2024-01-29T09:15:00Z",
  },
  {
    id: "item-16",
    companyId: "company-1",
    code: "ITEM-016",
    name: "Filing Cabinet 4-Drawer",
    description: "Metal filing cabinet with lock",
    itemType: "finished_goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Furniture",
    standardCost: 120.0,
    listPrice: 249.0,
    reorderLevel: 4,
    reorderQty: 10,
    isActive: true,
    createdAt: "2024-01-30T10:30:00Z",
    updatedAt: "2024-01-30T10:30:00Z",
  },
  {
    id: "item-17",
    companyId: "company-1",
    code: "ITEM-017",
    name: "Printer Paper A4 500 Sheets",
    description: "Premium white printer paper",
    itemType: "goods",
    uom: "BOX",
    uomId: getUomIdByCode("BOX"),
    category: "Office Supplies",
    standardCost: 4.5,
    listPrice: 8.99,
    reorderLevel: 50,
    reorderQty: 100,
    isActive: true,
    createdAt: "2024-01-31T11:45:00Z",
    updatedAt: "2024-01-31T11:45:00Z",
  },
  {
    id: "item-18",
    companyId: "company-1",
    code: "ITEM-018",
    name: "Network Router Gigabit",
    description: "High-speed wireless router",
    itemType: "goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Electronics",
    standardCost: 85.0,
    listPrice: 169.0,
    reorderLevel: 7,
    reorderQty: 15,
    isActive: true,
    createdAt: "2024-02-01T13:00:00Z",
    updatedAt: "2024-02-01T13:00:00Z",
  },
  {
    id: "item-19",
    companyId: "company-1",
    code: "ITEM-019",
    name: "Plastic Pellets HDPE",
    description: "High-density polyethylene pellets",
    itemType: "raw_material",
    uom: "KG",
    uomId: getUomIdByCode("KG"),
    category: "Raw Materials",
    standardCost: 1.8,
    listPrice: 3.2,
    reorderLevel: 1000,
    reorderQty: 5000,
    isActive: true,
    createdAt: "2024-02-02T14:15:00Z",
    updatedAt: "2024-02-02T14:15:00Z",
  },
  {
    id: "item-20",
    companyId: "company-1",
    code: "ITEM-020",
    name: "Conference Table 8-Seater",
    description: "Large conference table for meetings",
    itemType: "finished_goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Furniture",
    standardCost: 450.0,
    listPrice: 899.0,
    reorderLevel: 2,
    reorderQty: 5,
    isActive: true,
    createdAt: "2024-02-03T15:30:00Z",
    updatedAt: "2024-02-03T15:30:00Z",
  },
  {
    id: "item-21",
    companyId: "company-1",
    code: "ITEM-021",
    name: "External SSD 1TB",
    description: "Portable solid state drive",
    itemType: "goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Electronics",
    standardCost: 75.0,
    listPrice: 149.0,
    reorderLevel: 15,
    reorderQty: 30,
    isActive: true,
    createdAt: "2024-02-04T16:45:00Z",
    updatedAt: "2024-02-04T16:45:00Z",
  },
  {
    id: "item-22",
    companyId: "company-1",
    code: "ITEM-022",
    name: "Desk Lamp LED",
    description: "Adjustable LED desk lamp",
    itemType: "goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Office Supplies",
    standardCost: 18.0,
    listPrice: 39.0,
    reorderLevel: 20,
    reorderQty: 40,
    isActive: true,
    createdAt: "2024-02-05T08:00:00Z",
    updatedAt: "2024-02-05T08:00:00Z",
  },
  {
    id: "item-23",
    companyId: "company-1",
    code: "ITEM-023",
    name: "Whiteboard 6x4 ft",
    description: "Magnetic dry erase whiteboard",
    itemType: "goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Office Supplies",
    standardCost: 55.0,
    listPrice: 119.0,
    reorderLevel: 5,
    reorderQty: 10,
    isActive: true,
    createdAt: "2024-02-06T09:15:00Z",
    updatedAt: "2024-02-06T09:15:00Z",
  },
  {
    id: "item-24",
    companyId: "company-1",
    code: "ITEM-024",
    name: "Ergonomic Footrest",
    description: "Adjustable footrest for desk",
    itemType: "goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Office Supplies",
    standardCost: 12.0,
    listPrice: 29.0,
    reorderLevel: 25,
    reorderQty: 50,
    isActive: true,
    createdAt: "2024-02-07T10:30:00Z",
    updatedAt: "2024-02-07T10:30:00Z",
  },
  {
    id: "item-25",
    companyId: "company-1",
    code: "ITEM-025",
    name: "Power Strip 6 Outlets",
    description: "Surge protected power strip",
    itemType: "goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Electronics",
    standardCost: 8.0,
    listPrice: 19.0,
    reorderLevel: 30,
    reorderQty: 60,
    isActive: true,
    createdAt: "2024-02-08T11:45:00Z",
    updatedAt: "2024-02-08T11:45:00Z",
  },
  {
    id: "item-26",
    companyId: "company-1",
    code: "ITEM-026",
    name: "Glass Sheet Tempered 5mm",
    description: "Tempered glass for construction",
    itemType: "raw_material",
    uom: "SQM",
    uomId: getUomIdByCode("SQM"),
    category: "Raw Materials",
    standardCost: 45.0,
    listPrice: 75.0,
    reorderLevel: 50,
    reorderQty: 200,
    isActive: true,
    createdAt: "2024-02-09T13:00:00Z",
    updatedAt: "2024-02-09T13:00:00Z",
  },
  {
    id: "item-27",
    companyId: "company-1",
    code: "ITEM-027",
    name: "Office Desk L-Shape",
    description: "Modern L-shaped office desk",
    itemType: "finished_goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Furniture",
    standardCost: 280.0,
    listPrice: 599.0,
    reorderLevel: 3,
    reorderQty: 8,
    isActive: true,
    createdAt: "2024-02-10T14:15:00Z",
    updatedAt: "2024-02-10T14:15:00Z",
  },
  {
    id: "item-28",
    companyId: "company-1",
    code: "ITEM-028",
    name: "Noise Cancelling Headphones",
    description: "Wireless noise cancelling headphones",
    itemType: "goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Electronics",
    standardCost: 125.0,
    listPrice: 279.0,
    reorderLevel: 8,
    reorderQty: 15,
    isActive: false,
    createdAt: "2024-02-11T15:30:00Z",
    updatedAt: "2024-02-11T15:30:00Z",
  },
  {
    id: "item-29",
    companyId: "company-1",
    code: "ITEM-029",
    name: "Document Scanner",
    description: "High-speed document scanner",
    itemType: "goods",
    uom: "PC",
    uomId: getUomIdByCode("PC"),
    category: "Electronics",
    standardCost: 180.0,
    listPrice: 399.0,
    reorderLevel: 4,
    reorderQty: 10,
    isActive: true,
    createdAt: "2024-02-12T16:45:00Z",
    updatedAt: "2024-02-12T16:45:00Z",
  },
  {
    id: "item-30",
    companyId: "company-1",
    code: "ITEM-030",
    name: "Cleaning Service Monthly",
    description: "Monthly office cleaning service",
    itemType: "service",
    uom: "LICENSE",
    uomId: getUomIdByCode("LICENSE"),
    category: "Services",
    standardCost: 150.0,
    listPrice: 299.0,
    reorderLevel: 0,
    reorderQty: 0,
    isActive: true,
    createdAt: "2024-02-13T08:00:00Z",
    updatedAt: "2024-02-13T08:00:00Z",
  },
];
