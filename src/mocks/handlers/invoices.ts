...
export const invoiceHandlers = [
  ...
  http.get("/api/invoices", ({ request }) => {
    ...
    if (dateFrom) {
      filtered = filtered.filter((invoice) => new Date(invoice.invoiceDate) >= new Date(dateFrom));
    }
    if (dateTo) {
      filtered = filtered.filter((invoice) => new Date(invoice.invoiceDate) <= new Date(dateTo));
    }
  ...
  http.put("/api/invoices/:id", async ({ params, request }) => {
    const { id } = params;
    const body = (await request.json()) as Partial<CreateInvoiceRequest> & {
      status?: string;
      amountPaid?: number;
    };
