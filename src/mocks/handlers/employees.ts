...
import { employees, employeeDistributionLocations } from "../data/employees";
...
export const employeeHandlers = [
  ...
  http.put("/api/employees/:employeeId/territories/:territoryId", async ({ params, request }) => {
    ...
    const index = territoriesData.findIndex(
      (t) => t.id === territoryId && t.employeeId === employeeId
    );
    ...
    if (body.isPrimary) {
      territoriesData = territoriesData.map((t) =>
        t.employeeId === employeeId && t.id !== territoryId ? { ...t, isPrimary: false } : t
      );
    }
