...
type StockRequisitionLineInput =
  | CreateStockRequisitionRequest["items"][number]
  | NonNullable<UpdateStockRequisitionRequest["items"]>[number];
...
export class StockRequisitionLineValidationError extends Error {}

export type ResolvedStockRequisitionLineInput = StockRequisitionLineInput & {
...
export const resolveStockRequisitionLineUnitOptions = async (
  supabase: SupabaseLikeClient,
  companyId: string,
  items: StockRequisitionLineInput[]
): Promise<ResolvedStockRequisitionLineInput[]> => {
  ...
  return items.map((item) => {
    if (item.itemUnitOptionId) {
      ...
      if (!option) {
        throw new StockRequisitionLineValidationError("Selected unit option could not be found");
      }
      if (option.deleted_at) {
        throw new StockRequisitionLineValidationError("Selected unit option has been deleted");
      }
      if (!option.is_active) {
    ...
    if (!item.uomId) {
      throw new StockRequisitionLineValidationError("Unit is required for each requisition line");
    }
