...
export async function GET(request: NextRequest) {
  try {
    ...
    if (openingError) {
      return NextResponse.json({ error: "Failed to calculate opening balance" }, { status: 500 });
    }

    // Calculate opening balance based on account type
    let openingBalance = 0;
    if (openingLines && openingLines.length > 0) {
      const totalDebits = openingLines.reduce((sum, line) => sum + Number(line.debit), 0);
      const totalCredits = openingLines.reduce((sum, line) => sum + Number(line.credit), 0);
