  ...
  try {
    ...
    if (purchaseOrder.status !== "partially_received") {
      return NextResponse.json(
        { error: "Only partially received orders can be completed" },
        { status: 400 }
      );
    }
    ...
    if (updateError) {
      return NextResponse.json({ error: "Failed to complete purchase order" }, { status: 500 });
    }

    return NextResponse.json({
      success: true,
      message: `Purchase order ${purchaseOrder.order_code} marked as received`,
    });
  } catch {
