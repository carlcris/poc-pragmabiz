...
export async function POST(request: NextRequest) {
  try {
    ...
    const stockMap = new Map(
      stockData?.map((s) => [s.item_id, parseFloat(s.current_stock) || 0]) || []
    );
    ...
    for (let i = 0; i < body.payments.length; i++) {
      ...
      if (paymentError || !payment) {
        return NextResponse.json(
          { error: `Failed to record payment for ${paymentMethod.paymentMethod}` },
          { status: 500 }
        );
      }
    ...
    for (const item of invoiceItems || []) {
      ...
      const { data: stockTxItem, error: stockTxItemError } = await supabase
        ...
        .insert({
          ...
          total_cost:
            (parseFloat(String(item.quantity)) || 0) * (parseFloat(String(item.rate)) || 0),
