...
const userSafeReviewMessage = (message: string | undefined) => {
  ...
  return message && allowedMessages.has(message) ? message : "Failed to reject receiving overage";
};
...
async function POSTHandler(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (error) {
      console.error("Error rejecting delivery note receiving overage:", error);
    ...
    console.error("Unexpected error rejecting delivery note receiving overage:", error);
...
export const POST = withActivityLogging(POSTHandler, {
  ...
  route: "/api/delivery-notes/[id]/receiving-overages/[itemId]/reject",
});
