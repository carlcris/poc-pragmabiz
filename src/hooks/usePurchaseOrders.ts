...
export function useSubmitPurchaseOrder() {
  ...
  return useMutation({
    mutationFn: (id: string) => purchaseOrdersApi.submitPurchaseOrder(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [PURCHASE_ORDERS_QUERY_KEY] });
    },
  });
...
export function useApprovePurchaseOrder() {
  ...
  return useMutation({
    mutationFn: (id: string) => purchaseOrdersApi.approvePurchaseOrder(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [PURCHASE_ORDERS_QUERY_KEY] });
    },
  });
...
export function useCompletePurchaseOrder() {
  ...
  return useMutation({
    mutationFn: (id: string) => purchaseOrdersApi.completePurchaseOrder(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [PURCHASE_ORDERS_QUERY_KEY] });
    },
  });
