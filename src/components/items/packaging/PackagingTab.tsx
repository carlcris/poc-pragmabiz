...
export const PackagingTab = ({ itemId, readOnly = false }: PackagingTabProps) => {
  ...
  const { data: packagingData, isLoading: packagingLoading } = useQuery<PackagingResponse>({
    ...
    queryFn: async () => {
      const response = await apiClient.get<PackagingResponse>(`/api/items/${itemId}/packages`);
      return response;
    },
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (packagingId: string) => {
      await apiClient.delete(`/api/items/${itemId}/packages/${packagingId}`);
    },
    onSuccess: () => {
        ...
        <CardContent className="space-y-4">
          ...
          {packagingLoading ? (
            ...
            <div className="py-8 text-center text-muted-foreground">
              ...
              <p className="mt-2 text-sm">Add your first packaging option to get started.</p>
            </div>
          ) : (
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-1">
                            ...
                            <Button
                              ...
                              title={
                                pkg.isDefault ? "Cannot delete base packaging" : "Delete packaging"
                              }
                            >
                              <Trash2
                                className={`h-4 w-4 ${pkg.isDefault ? "text-muted-foreground" : "text-destructive"}`}
                              />
                            </Button>
                          </div>
                        </TableCell>
