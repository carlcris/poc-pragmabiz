...
export const createIntegrationSettingsSchema = () =>
  z.object({
    ...
    webhook_url: z.string().url("Invalid webhook URL").max(500).optional().or(z.literal("")),
    webhook_secret: z.string().max(255).optional().or(z.literal("")),
    accounting_integration: z.enum(["none", "quickbooks", "xero", "sage"] as const),
    accounting_sync_enabled: z.boolean().default(false),
    ecommerce_integration: z.string().max(100).optional().or(z.literal("")),
  });
