...
export const createWorkflowSettingsSchema = () =>
  z.object({
    ...
    po_approval_threshold: z.number().min(0, "Threshold must be 0 or greater"),
    po_auto_approve_below: z.number().min(0, "Auto-approve threshold must be 0 or greater"),
    stock_request_approval_required: z.boolean().default(false),
    sr_approval_threshold: z.number().min(0, "Threshold must be 0 or greater"),
