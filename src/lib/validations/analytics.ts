...
// Enums
export const timePeriodEnum = z.enum(["daily", "weekly", "monthly", "quarterly", "yearly"]);

export const exportFormatEnum = z.enum(["excel", "pdf"]);

export const reportTypeEnum = z.enum(["overview", "by-employee", "by-location", "by-time", "all"]);
...
export const salesAnalyticsFilterSchema = z.object({
  ...
  cities: z.array(z.enum(MINDANAO_CITIES as unknown as [string, ...string[]])).optional(),
  regionState: z.enum(MINDANAO_REGIONS as unknown as [string, ...string[]]).optional(),
  regions: z.array(z.enum(MINDANAO_REGIONS as unknown as [string, ...string[]])).optional(),
  timePeriod: timePeriodEnum.optional().default("daily"),
});

export type SalesAnalyticsFilterValues = z.infer<typeof salesAnalyticsFilterSchema>;
