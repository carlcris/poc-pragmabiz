...
export const purchaseReceiptsApi = {
  ...
  getReceipts: async (filters?: PurchaseReceiptFilters): Promise<PurchaseReceiptListResponse> => {
    const params = new URLSearchParams();

    if (filters?.search) params.append("search", filters.search);
    if (filters?.status && filters.status !== "all") params.append("status", filters.status);
    if (filters?.purchaseOrderId) params.append("purchase_order_id", filters.purchaseOrderId);
  ...
  updateReceipt: async (id: string, data: UpdatePurchaseReceiptRequest): Promise<void> => {
    const response = await fetch(`${API_BASE}/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data),
